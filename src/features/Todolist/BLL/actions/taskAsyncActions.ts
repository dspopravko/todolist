...
const getTasks = createAsyncThunk<{ tasks: TaskType[]; todolistId: string },
  string,
  ThunkError>('tasks/getTasks', async (todolistId, thunkAPI) => {
  try {
...
const addTask = createAsyncThunk<TaskType,
  { title: string; todolistId: string },
  ThunkError>('tasks/addTask', async (param, thunkAPI) => {
  try {
...
const updateTask = createAsyncThunk(
  ...
  async (
    ...
    if (!task) {
      return thunkAPI.rejectWithValue('task not found in the app state')
    }

...
test("correct task should be added to correct array", () => {
  ...
  const action = addTask.fulfilled(param, 'requestId', { todolistId: todolistId2, title: 'Juice' });
...
test("new array should be added when new todolist is added", () => {
  ...
  const action = addTodolist.fulfilled(param.todolist, 'requestId', { title: "new todolist" });
  const endStateTasks = tasksReducer(tasksState, action)

  const todolistsId = Object.keys(endStateTasks);
  const addedTodolistId = todolistsId.find(k => k != todolistId1 && k != todolistId2 && k != todolistId3);
